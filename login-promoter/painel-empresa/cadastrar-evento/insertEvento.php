...
require_once('/xampp/htdocs/events4u/config.php');
...
if(mysqli_num_rows($resultId) > 0) {
    ...
    $resultArray = mysqli_fetch_assoc($resultId);
        ...
        if(isset($_POST['submit'])) {
            ...
            $verifica = mysqli_query($conexao, "SELECT * FROM eventos WHERE id_empresa = '{$id_empresa}' AND local_evento = '{$endereco}' AND data_evento = '{$data}'");
            ...
            $result = mysqli_query($conexao, "INSERT INTO eventos (nome, tipo_evento, data_evento, local_evento, cidade, n_seguidores, link_venda, id_empresa) VALUES('$nome', '$tipo', '$data', '$endereco', '$cidade', '$seguidores', '$venda', '$id_empresa')") or die($mysqli -> error);
