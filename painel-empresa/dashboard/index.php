...
<?php
           ...
           $resultEventos = mysqli_query($conexao, "SELECT * FROM eventos WHERE id_empresa = {'$id_empresa'} AND online = '1'");
                ...
                if(mysqli_num_rows($resultEventos) > 0) {
                    ...
                    $sqlPromoterSolicita = mysqli_query($conexao, "SELECT * FROM usuarios_solicita WHERE id_evento = '{$id_evento}'");

                    $rowsSolicita = mysqli_num_rows($sqlPromoterSolicita);

                    $sqlPromoter = mysqli_query($conexao, "SELECT * FROM usuarios_eventos WHERE id_evento = '{$id_evento}'");

                    $rows = mysqli_num_rows($sqlPromoter);
                    ...
                    if(mysqli_num_rows($resultFolder) > 0) {
                        ...
                        $id_folder = $resultArrayFolder ['id_folder'];
...
?>    
                    <div class="catalogo">
                        ...
                        <!-- Conteudo -->
                        <div class="conteudo">
                            <p class="post_titulo">Nome: <a class="nome_evento"><?php echo $user_data['nome']; ?></a></p>
                            ...
                            <p class="post_local">Solicitação dos promoters pendentes: <?php echo $rowsSolicita ?></p>
                            <p class="post_local">
                                Promoters participando: <?php echo $rows ?>
                            </p>

                            <div class="btn">
                            <a class="btn_evento" href="info.php?<?php echo "id=".$user_data['id_evento']?>">SAIBA MAIS</a>

                            <a href="../cadastrar-evento/delete.php?<?php echo "id=".$user_data['id_evento'] ?>" title="Excluir o evento">
